...
export class AppComponent {
	...
	move(event: any): void {
		...
		const right = parseInt(element.right, 10);
		const bottom = parseInt(element.bottom, 10);
		...
		switch (keyCode) {
			...
			case 39:
				this.moveRight(left + this.POSITION_SHIFT_BY, right, top); // right
				break;
			...
			case 40:
				this.moveDown(left, bottom, top + this.POSITION_SHIFT_BY); // down
				break;
			case 65:
				this.moveLeft(left - this.POSITION_SHIFT_BY, top); // left
				break;
	...
	moveRight(left, right, top): void {
		const boxWrapperPosition = document
			.getElementById('box-area')
			.getBoundingClientRect();

		if (boxWrapperPosition.right > left && boxWrapperPosition.right >= right + this.POSITION_SHIFT_BY) {
			this.updatePosition(this.selectedBox, left, top);
		}
	}
	...
	moveDown(left, bottom, top): void {
		const boxWrapperPosition = document
			.getElementById('box-area')
			.getBoundingClientRect();

		if (boxWrapperPosition.bottom > top && boxWrapperPosition.bottom >= bottom + this.POSITION_SHIFT_BY) {
			this.updatePosition(this.selectedBox, left, top);
		}
	}
}
